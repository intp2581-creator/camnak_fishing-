...
class _StoreScreenState extends State<StoreScreen> {
  ...
  // 🪱 상점에서 내 보유 수량 조회(미끼 등) — myInventory(구매 즉시 갱신)에서 이름 매칭 합산.
  int _ownedQty(String name) {
    int q = 0;
    for (final i in myInventory) {
      if (i is Map && (i['name'] ?? '').toString() == name) {
        q += (i['quantity'] is num) ? (i['quantity'] as num).toInt() : 1;
      }
    }
    return q;
  }
                          ...
                          Expanded(
                            ...
                            child: ElevatedButton(
                              ...
                              onPressed: !auxLvOk ? null : () {
                                ...
                                // 🧵 낚싯줄도 소모품이다. 장비로 묶여 있어 '이미 보유 중'에 막히는 바람에
                                //    여분을 못 사고, 낚시 중 끊어지면 대기실까지 나갔다 와야 했다
                                //    (2026-09-09 달빛둠벙님 요청). 미끼·입장권과 같이 재구매·스택 허용.
                                bool isLine = itemType == 'LINE' || itemName.contains('낚싯줄') || itemName.contains('낚시줄');
                                int price = item['price'];
                                if (isBait || isTicket || isLine) { showDialog(context: context, builder: (context) => AlertDialog(backgroundColor: Colors.grey.shade900, title: Text(isTicket ? '🎟️ 입장권 구매' : (isLine ? '🧵 낚싯줄 구매' : '🛒 미끼 구매'), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), content: Text('$itemName 구매로 $price K가 차감됩니다.\n구매하시겠습니까?', style: const TextStyle(color: Colors.white70)), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('취소', style: TextStyle(color: Colors.grey))), TextButton(onPressed: () { Navigator.pop(context); _buyItem(item); }, child: const Text('확인', style: TextStyle(color: Color(0xFFD4AF37), fontWeight: FontWeight.bold)))])); } else {
