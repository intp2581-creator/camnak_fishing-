            ...
            child: Padding(
              ...
              child: Builder(
                builder: (context) {
                  ...
                  if (isMallOnly) {
                    // 🧪 [테스트용] '하수 조사'만 실제 camnak.com 쇼핑몰 결제 링크를 켜둠. 테스트 끝나면 이 if 블록만 지워서 원복!
                    if (itemName.contains('하수')) {
                      const String hasuProductUrl = 'https://camnak.com/shop_view/?idx=224';
                      return Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.stretch, children: [const Center(child: Text('💳 쇼핑몰 결제', style: TextStyle(color: Color(0xFFD4AF37), fontSize: 16, fontWeight: FontWeight.w900))), const SizedBox(height: 10), ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFD4AF37), foregroundColor: Colors.black, padding: const EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))), onPressed: () { audioManager.playSfx("sfx_click.mp3"); html.window.open(hasuProductUrl, '_blank'); }, child: const Text('🛒 쇼핑몰에서 구매', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)))]);
                    }
