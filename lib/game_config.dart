...
const List<String> garamFwFish = ['붕어', '잉어', '가물치', '메기', '떡붕어', '강준치', '블루길', '베스', '살치', '자라', '쏘가리', '꺽지', '무지개송어', '향어', '민물장어', '동자개', '누치', '끄리', '초어', '모래무지'];
const List<String> garamSeaFish = ['참돔', '감성돔', '광어', '우럭', '갈치', '고등어', '벵에돔', '갑오징어', '주꾸미', '문어', '참치', '볼락', '학꽁치', '성대', '농어', '부시리', '돌돔', '방어', '쥐노래미', '무늬오징어'];
...
// ═══════════════════════════════════════════════════════════════════
// 💎 [보석] 제나스(바다광장 보석 교환소)에게 잡은 물고기를 주고 받는다.
//    낚싯대 공방(바르탄)에서 KREFT 낚싯대를 만들 때 쓰는 재료다.
//    ⚠️ 같은 어종끼리만 모아야 한다 — 붕어 5 + 잉어 5 로는 안 되고 붕어 10 이어야 한다.
//       "이 물고기를 노려서 잡는다"는 목표가 생기라고 이렇게 했다(2026-09-05 사용자 결정).
// ═══════════════════════════════════════════════════════════════════

/// 보석 등급(낮음 → 높음). 상점·인벤 정렬에도 이 순서를 쓴다.
const List<String> kGemOrder = ['크리스탈', '자수정', '사파이어', '루비', '다이아몬드'];

const Map<String, String> kGemIcon = {
  '크리스탈': 'gem_crystal.png',
  '자수정': 'gem_amethyst.png',
  '사파이어': 'gem_sapphire.png',
  '루비': 'gem_ruby.png',
  '다이아몬드': 'gem_diamond.png',
};

/// 보석 1개를 만드는 '조합'. 적힌 어종을 전부 그 수만큼 모아야 한 개가 나온다.
///   예) 다이아몬드 = 6대장을 각각 10마리씩, 합쳐서 60마리.
///   한 어종만 잡아선 못 만들고 골고루 잡아야 한다(2026-09-05 사용자 설계).
///   ⛔ 여기 없는 어종(메기·끄리·베스·동자개·모래무지·볼락·갈치·벵에돔·쥐노래미,
///      자라·고등어)은 교환 대상이 아니다 → 상점에 팔아 KREFT로 쓴다.
//   📐 등급마다 난이도가 계단이 되게 잡는다(2026-09-10 재조정).
//      예전엔 크리스탈·자수정·사파이어가 전부 5종×5마리라 난이도가 똑같았고,
//      루비는 초어(전설, 한 달 1.6마리)를 5마리나 요구해 다이아몬드보다 어려웠다.
//      하위 셋은 어종만 다르고 각 5마리로 같다(등급 = 어종 구성).
//      루비 7마리+전설1, 다이아몬드 10마리+전설2 로 뒤가 무거워진다.
//      25 · 25 · 25 → 43(+전설1) → 62(+전설2)
const Map<String, Map<String, int>> kGemRecipe = {
  // 💎 다이아몬드 — 6대장 각 10마리 + 전설 둘(초어·참치) 각 1마리. 최종 재료(1~2개월)
  //    ⚠️ 낚싯대 레시피에서 여러 개를 요구하지 말 것. 하나가 두 달짜리다.
  '다이아몬드': {'붕어': 10, '잉어': 10, '가물치': 10, '참돔': 10, '감성돔': 10, '문어': 10,
    '초어': 1, '참치': 1},
  // ❤️ 루비 (민물4 · 바다3) — 초어는 1마리만. 쏘가리·꺽지가 함께 들어가 '루어로 노리는 보석'
  '루비': {'향어': 7, '쏘가리': 7, '꺽지': 7, '초어': 1, '방어': 7, '부시리': 7, '무늬오징어': 7},
  // 💙 사파이어 (민물2 · 바다3)
  '사파이어': {'민물장어': 5, '누치': 5, '돌돔': 5, '농어': 5, '갑오징어': 5},
  // 💜 자수정 (민물3 · 바다2)
  '자수정': {'떡붕어': 5, '강준치': 5, '무지개송어': 5, '우럭': 5, '주꾸미': 5},
  // 🤍 크리스탈 (민물2 · 바다3) — 가장 가벼운 보석. 1존 레이드 보상과 값이 맞는다
...
String? gemUsing(String fishName) {
  for (final e in kGemRecipe.entries) {
    if (e.value.containsKey(fishName)) return e.key;
  }
  return null;
}

/// 인벤토리에 넣을 보석 아이템 한 개
Map<String, dynamic> makeGem(String gemName, {int qty = 1}) => {
      'name': gemName,
      'price': 0,
      'category': 'COMMON',
      'type': 'GEM',
      'quantity': qty,
      'icon': kGemIcon[gemName] ?? 'gem_crystal.png',
      'desc': '낚싯대 공방에서 KREFT 낚싯대를 만들 때 쓰는 재료예요.',
    };
...
// 📍 핫스팟에서 '귀한 어종'이 더 자주 나오는 배율.
//   대상은 기본 가중치 15 이하 = 참치(5)·자라(5)·방어(10)·초어(10) 넷뿐.
//   나머지 어종은 전부 25·30·50 이라 해당 없다.
//
//   1.5배인 이유: 2배로 두면 고등어 미끼로 선상에 나갔을 때 방어가
//   12%→21% 로 뛴다. 다섯 마리에 한 마리가 방어라 티가 너무 난다.
//   1.5배면 17% 로 자연스럽고 체감은 충분하다(초어 +46%, 참치 +54%).
//   ⚠️ 가중치가 정수라 w=5 는 7.5→8 로 반올림돼 실제로는 1.6배가 된다.
const double kHotSpotRareMult = 1.5;

//   자라 제외 — 15~30cm 잡어라 '대물 기회'라는 취지와 무관하다.
//   빼기 전에는 핫스팟 혜택의 절반을 자라가 먹고 있었다.
const Set<String> kHotSpotRareExclude = {'자라'};

// 🎰 [전설 어종 확률문] 참치·초어는 '한 달에 한두 마리' 급으로 둔다.
//   여기 적힌 확률에 걸린 입질에서만 후보에 오른다(나머지 입질에선 아예 빠진다).
//
//   ⚠️ 왜 미끼 배율로 안 하나 — 가중치가 정수라 아무리 낮춰도 최소 1에서 멈추고,
//      생미끼는 어종 풀이 좁아(합계 600대) 가중치 1도 한 달 13마리나 된다.
//      0.1 로 더 낮추면 반올림해서 0이 되어 영영 안 나온다. 중간이 없다.
//
//   ⚠️ 어종마다 값이 다른 이유 — 그냥 두면 나오는 빈도가 40배 차이 난다.
//      참치: 가중치 5, 생미끼(고등어·오징어)로만 물림 → 한 달 78마리
//      초어: 가중치 10, 흔한 옥수수 상성 2.0 + 저수지 보정 1.3 → 한 달 405마리
//
//   기준: 하루 1시간 × 30일 = 한 달 8,000입질. 아래 값이면 둘 다 월 1~2마리.
//   더 자주 나오게 하려면 숫자만 올리면 된다. 미끼표는 건드릴 필요 없다.
//   미끼와 무관하게 걸리므로 특정 미끼로 캐낼 수 없다.
const Map<String, double> kLegendAppearRate = {
  '참치': 0.02,    // 고등어 미끼 기준 한 달 1.6마리
  '초어': 0.004,   // 옥수수 기준 한 달 1.6마리
};

// 🎁 [전설 어종 별도 보상] 잡으면 이만큼을 '더해준다'. 잭팟 ×3 과 별개.
//
//   ⚠️ 왜 배율이 아니라 정액인가 — 한 마리당 보상이 워낙 작아서(★5 우럭 103 KREFT)
...
/// 🧰 제작 재료인가 — 보석(GEM). 나중에 주문서 등이 늘면 여기만 고치면 된다.
