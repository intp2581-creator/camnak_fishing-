...
class _PlazaScreenState extends State<PlazaScreen> with SingleTickerProviderStateMixin {
  ...
  static const Map<String, List<double>> kRaidFlagSlots = {
    // 📐 [2026-09-02] 기준선을 0.50 → 0.516으로. 배경 그림의 방 중심(길드명 나무 간판)이
    //    0.516이고 좌우 방패가 0.380·0.641인데, 깃발만 0.50 기준이라 오른쪽이 간판에
    //    0.032만큼 더 붙어 보였다. 간판 기준 대칭으로 다시 잡으니 그려진 방패와도 맞는다.
    'murgadon': [0.376, 0.47, 0.25], // 좌(1단계) — 방패 자리(0.380)에서 살짝 더 왼쪽.
                                    // 좌우 거리는 같지만 오른쪽에 여울 NPC가 서 있어
                                    // 그쪽이 무거워 보인다 → 눈으로 맞춘 값(2026-09-02)
    'abykura':  [0.646, 0.47, 0.25], // 우(2단계) — 푸른 방패 자리(0.641)
    'basragon': [0.236, 0.50, 0.26], // 앞쪽 좌(3단계)
    'kargon':   [0.796, 0.50, 0.26], // 앞쪽 우(4단계)
    'volkar':   [0.516, 0.50, 0.26], // 중앙 앞(5단계) — 간판 정중앙
  };
  ...
  void _tapTick() {
    ...
    const speedPxPerSec = 200.0; // 조이스틱과 동일 속도
    const dt = 16 / 1000.0;
  ...
  void _joyTick() {
    ...
    const speedPxPerSec = 200.0; // 월드 스크린px 기준 이동 속도(절반으로 느리게)
    const dt = 16 / 1000.0;
  ...
  Widget _chatTabBtn(int index, String title) {
    ...
    final Widget btn = GestureDetector(
      onTap: () => setState(() {
        ...
        if (index == 4) _readSystemAt = DateTime.now();  // 시스템 열면 읽음
      }),
      child: Container(
    ...
    // 🔴 안 읽은 귓속말(1)/길드(3)/시스템(4) 뱃지 — 그 탭을 보고 있지 않을 때만
    if ((index == 1 || index == 3 || index == 4) && !active) {
  ...
  Widget _unreadBadge(int index) {
    // 🧾 시스템: 결제·선물로 물건이 들어온 걸 못 보고 지나치지 않게 뱃지를 띄운다.
    if (index == 4) {
      final uid = FirebaseAuth.instance.currentUser?.uid;
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                // 🧾 시스템 탭은 읽기만 하는 곳 — 입력칸을 감춘다(칠 데가 없어야 안 헷갈린다)
                if (_chatTab != 4) SizedBox(
                  height: 34,
                  child: TextField(
                            ...
                            child: IgnorePointer(
                              child: AnimatedBuilder(
                                ...
                                builder: (context, _) {
                                  ...
                                  final bob = _walking ? math.sin(phase).abs() * 2.0 : 0.0; // 걸음 상하 바운스
  ...
  void _openBoxDialogPlaza(Map<String, dynamic> box, void Function(void Function()) setD) {
    ...
    if ((box['name'] ?? '') == kGiftBoxName || box['gift'] is List) { _openGiftDialogPlaza(box, setD); return; }
  ...
  void _openGiftDialogPlaza(Map<String, dynamic> box, void Function(void Function()) setD) {
    ...
    showDialog(context: context, barrierDismissible: true, builder: (dctx) => AlertDialog(
      backgroundColor: const Color(0xFF1A1A1A),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14), side: const BorderSide(color: _kGold, width: 1.4)),
      title: Text('${(box['name'] ?? '') == kGiftBoxName ? '🎁' : '📦'} $title', style: const TextStyle(color: _kGold, fontSize: 18, fontWeight: FontWeight.bold)),
